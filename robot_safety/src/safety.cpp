                       ...
                       node_loop_rate_ (20)
{
	...
	cmd_vel_sub_ = nh_.subscribe("/cmd_vel", 1, &Safety::CmdVelCallback, this);
	bumper_sub_ = nh_.subscribe("/bumper", 1, &Safety::bumperCallback, this);
	...
	for(int i = 0; i < scan_num_; i++)
	{
		...
		scan_sub_[i] = nh_.subscribe<sensor_msgs::LaserScan>(ScanTopicName.c_str(), 10, boost::bind(&Safety::scanCallback, this, _1, i));
	}

	dynamic_reconfigure::Server<robot_safety::RobotSafetyConfig>::CallbackType f;
    f = boost::bind(&Safety::reconfig_callback, this, _1, _2);
...
void Safety::reconfig_callback(robot_safety::RobotSafetyConfig &config, uint32_t level)
{
...
void Safety::buildRectangleVisgs()
{
...
std::vector<geometry_msgs::Point> Safety::draw_range(double rang_w, double rang_l)
{
...
void Safety::CmdVelCallback(const geometry_msgs::TwistConstPtr& msg)
{
...
void Safety::bumperCallback(const std_msgs::BoolConstPtr& msg)
{
	if(msg->data)
	{
...
void Safety::scanCallback(const sensor_msgs::LaserScan::ConstPtr &msg, int idx)
{
...
void Safety::check(sensor_msgs::PointCloud cloud, int idx)
{
...
void Safety::inE2(geometry_msgs::Point32 point, int idx)
{
...
double Safety::solveE1(geometry_msgs::Point32 point, int idx)
{
	double dis_vel;
...
void Safety::visualizeRectangles(bool show)
{
	stop_viz_msg_.header.stamp = slow_viz_msg_.header.stamp = ros::Time::now();
